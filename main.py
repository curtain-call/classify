...
def train(**kwargs):
    ...
    model.fc = nn.Linear(512, 100).cuda()
    # if opt.load_model_path:
    #     model.load(opt.load_model_path)
    if opt.use_gpu:  # GPU
        divice = torch.cuda.set_device(0)
        model.to(divice)
        ...
        for ii, (data, label) in enumerate(train_dataloader):
            ...
            if ii % opt.print_freq == opt.print_freq - 1:
                ...
                if os.path.exists(opt.debug_file):
                    ...
                    ipdb.set_trace()
        # model.save()
        name = time.strftime('model' + '%m_%d_%H_%M_%S.pth')
...
def test(**kwargs):
    ...
    model.load_state_dict(t.load('./checkpoints_two_classification/model07_13_09_44_01.pth'))
    if opt.use_gpu:
        model.cuda()
    model.eval()
